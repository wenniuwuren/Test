...
public class Chapter06 {
    ...
    public void run() throws InterruptedException, IOException
    {
        ...
        try {
            Jedis conn = new Jedis(Constants.REDIS_HOST_COMPANY, 6379);
            conn.auth("redis"); //如果Redis服务连接需要密码，制定密码
            conn.select(15);
...
//        testFileDistribution(conn);
    ...
    /**
     * (1) 如果指定联系人已经存在于最近联系人,那么从给列表里面删除他 lrem
     * (2) 将指定的联系人添加到最近联系人列表的最前面 lpush
     * (3) 如果在添加操作完成后，最近联系人列表包含联系人超过100个，那么进行trim，只保留前100个 ltrim
     * @param conn
     * @param user
     * @param contact
     */
    public void addUpdateContact(Jedis conn, String user, String contact) {
        ...
        // Count > 0 从表头删除;Count < 0 从表尾删除;count = 0 移除表中所有与 value 相等的值。
    ...
    public List<String> fetchAutocompleteList(Jedis conn, String user, String prefix) {
        ...
        for (String candidate : candidates) { // 前缀符合就加入最终结果
